...
export default function ProjectsBoard() {
    const { snapshot, partnerMap, circleMap, meetingMap, openMeeting, addProject, projectBoardCircleId, setProjectBoardCircleId } =
        useWorkspaceSnapshot();
    const [showAddModal, setShowAddModal] = useState(false);
    const [newProjectTitle, setNewProjectTitle] = useState("");
    const [newProjectRoleId, setNewProjectRoleId] = useState("");

    const activeCircleId = circleMap[projectBoardCircleId] ? projectBoardCircleId : Object.keys(circleMap)[0];
    ...
    return (
        <div className="flex h-full flex-col gap-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
                <div className="flex flex-wrap items-center gap-1.5 rounded-full
                    ...
                    {snapshot.circles.map((circle) => {
                        ...
                        return (
                            <button key={circle.id} type="button"
                                onClick={() => { setProjectBoardCircleId(circle.id); setNewProjectRoleId(""); }}
                            ...
                            <section key={col.id} aria-labelledby={`col-${col.id}`}
                                ...
                                <div className="mb-3 flex items-center justify-between px-1">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="rounded-full bg-slate-100 dark:bg-white/[0.06]
                                            px-2 py-0.5 text-[10px] font-semibold text-slate-500 dark:text-slate-500">
                                            {cards.length}
                                        </span>
                                    </div>
                                    <button type="button" aria-label={`More options for ${col.title}`}
                                ...
                                {/* Cards */}
                                <div className="custom-scrollbar flex flex-1 flex-col gap-2.5 overflow-y-auto
                                    rounded-2xl border border-slate-200/60 dark:border-white/[0.06]
                                    bg-slate-50/60 dark:bg-white/[0.02]
                                    p-2.5">
                                    {cards.map((project) => {
                                        const sourceMeeting = project.sourceMeetingId ? meetingMap[project.sourceMeetingId] : null;
                                        const owner = project.ownerId ? partnerMap[project.ownerId] : null;
                                        return (
                                            <article key={project.id}
                                                ...
                                                style={{ transitionTimingFunction: SPRING }}>
                                                {/* Accent bar */}
                                                <div className={`mb-3 h-1 w-10 rounded-full ${project.accentToken}`} />
                                                ...
                                                {sourceMeeting && (
                                                    <button type="button" onClick={() => openMeeting(sourceMeeting.id)}
                            ...
                            <div className="rounded-[calc(1.75rem-5px)] p-6">
                                ...
                                <div className="flex flex-col gap-4">
                                    <div>
                                        ...
                                        <input type="text" value={newProjectTitle}
