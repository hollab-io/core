...
import { Info, Menu, Search, Sun, Moon } from 'lucide-react';
import { TAB_TITLES, type AppTabId } from '../config/navigation';
...
export default function Topbar({ activeTab, searchQuery, setSearchQuery, isDarkMode, setIsDarkMode }: TopbarProps) {
  ...
  const renderSegmentedControl = (
    ...
    <div className="ml-0 flex items-center gap-2 rounded-lg bg-transparent p-1 sm:ml-6" role="tablist">
      {options.map((option) => {
    ...
    <header className="sticky top-0 z-10 border-b border-border/60 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 px-4 py-4 shadow-sm backdrop-blur-md sm:px-8 transition-colors duration-300">
      <div className="flex min-h-12 flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center justify-end gap-3 sm:gap-4">
          {(activeTab === 'chart' || activeTab === 'projects' || activeTab === 'actions') && (
