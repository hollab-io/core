...
/** Encode an Election change (assign role lead): abi.encode(roleId, newLead, previousLead) */
export function encodeElection(
    roleId: bigint,
    lead: `0x${string}`,
    previousLead: `0x${string}` = "0x0000000000000000000000000000000000000000",
): `0x${string}` {
    return encodeAbiParameters(
        [{ type: "uint256" }, { type: "address" }, { type: "address" }],
        [roleId, lead, previousLead],
    );
}
